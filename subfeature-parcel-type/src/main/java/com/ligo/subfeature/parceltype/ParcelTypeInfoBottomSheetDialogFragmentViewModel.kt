...
class ParcelTypeInfoBottomSheetDialogFragmentViewModel(
    navigator: INavigator,
    analytics: IAnalytics,
    private val remoteConfig: IRemoteConfig,
) : BaseViewModel(navigator, analytics) {

    private val parcelTypeUiListSubject: Subject<List<ParcelTypeUiModel>> = PublishSubject.create()

    fun fetchParcelTypeUiList() {
        val parcelTypeUiList = remoteConfig.getParcelTypeList()
            .map { ParcelTypeUiModel.fromRemoteConfigParcelType(it) }
